...
class CreateBrand
{
    ...
    public function handle(CreateBrandCommand $dto): BrandSimpleDTO
    {
        if (is_null($dto->code)) {
        ...
        $entity = BrandEntity::create(
            new NameVO($dto->name),
            new BrandCode($dto->code)
        );

        if ($this->repository->isDuplicate($entity)) throw new DomainConflictException("Merk '".$entity->getName()."' dengan code '".$entity->getCode()."' sudah tersedia");
